...
# This class describes the response that our API sends back to the client.
class TextResponse(BaseModel):
    task: str # The API response tells the client which task was performed. For example, "task": "summarize"
    result: str # This contains the actual response generated by the LLM.
    model: str # This tells us which LLM model generated the result.
    # tokens_used is optional; when available it should be an integer, otherwise its value can be None.
    # Optional type hint: tokens_used can be an integer or None.
    # None is used when token usage information is unavailable.
    tokens_used: Optional[int] = None 

# One more Pydantic model HealthResponse to manage validation and serialization for this response.
# Defines the JSON structure returned by the /health endpoint.
# In main.py we have @app.get("/health", response_model=HealthResponse) -  ths tells FastAPI, the response from /health should follow the HealthResponse structure.
class HealthResponse(BaseModel):
    status: str # status should contain a string. For example, "status": "healthy"
    provider: str # This tells the client which LLM provider the application is using.
    model: str
